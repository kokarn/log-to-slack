...
apache2.prototype.getMessageInfo = function( message ){
    ...
    refererPosition = messageInfo.message.indexOf( ', referer:' );

    // Add referer field
    if( refererPosition > -1 ){
        messageInfo.extraFields.push({
            title: 'Referer',
            value: messageInfo.message.substr( refererPosition + 10 ).trim()
        });

        messageInfo.message = messageInfo.message.substring( 0, refererPosition );
    }
